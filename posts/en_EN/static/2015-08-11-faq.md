...
Duniter uses cryptography features provided by [Ed25519](http://www.wikiwand.com/en/EdDSA) scheme to both:
...
No, all Duniter data is public & replicated. Anyone using Duniter may have a complete copy of the database including both community & transactions.
...
The relative theory of money is a book published by Stephane Laborde which explains what is a free money, how the parameters are computed and an history of money. This book is a theory which requires mathematical and economic knowledge. [This introduction to the relative theory of money](http://cuckooland.free.fr/TheRtmForTheKids.html), maybe easier to understand, to start with.
...
You can transfer money by using _Transactions_. Transactions are just like cheques: you write who is the issuer, the recipient, and send it to the network. When it is validated, money has been wired. However, transactions can do more than simple check: it may have any number of sources (minimum 1) and any number of recipients (minimum 1). Also, it exists 2 different transaction sources:
...
At any moment, the common data structure of Duniter (the _blockchain_) allows to know it: membership is simply written in it. Anyone may check it.
...
Yes, just like Bitcoin and other altcoins, Duniter stores its data in a blockchain. A blockchain is a succession of blocks which are stacked, together, sequentially. Each block contains information about members & transactions. A chain of blocks gives 2 major information, for any _t _time:
...
First because for the currency to work, validators are required: they are the people _validating_ the blockchain, by writing new blocks on top of it. Since blocks define members & transactions, validators are those making the currency dynamic by allowing us creating new money and wired it to someone else, or even receive money from others. Without validators, there _can't_ be currency.
### Other motivations?
...
Precise rule is given in [the protocol](https://github.com/duniter/duniter/blob/master/doc/Protocol.md#proof-of-work). You should check it out, you will have a lot of technical informations about Duniter works.
...
You can help on project development in many ways, depending your available time, energy and your profile. Please read below requirements and possibilities. If you feel comfortable with one these profiles, do not hesitate to [contact us at admin@duniter.org(mailto:admin@duniter.org).
...
If you are not afraid by those technologies, you can both start reading [HTTP API](https://github.com/duniter/duniter/blob/master/doc/HTTP_API.md) and [contact us](mailto:admin@duniter.org).
...
Well, if reached this paragraph reading the whole FAQ, you should have a rather good idea of what is Duniter. You could go deeper with [Theoretical section](http://duniter.org/theoretical). From there, you should be able to talk about the project with a minimum of background. If you want to get deeper in Duniter, read [UCP](http://duniter.org/specs), this is Duniter protocol. All what you need to know about technical IT stuff is there, even if not in a tutorial format. Last, you could try to read [RMT](http://en.trm.creationmonetaire.info/), which is the fundamental theory behind Duniter's way of creating new money.
### The best way to stay tuned?

[Mailing List](https://groups.google.com/forum/?hl=fr#!forum/duniter) for major events, [Duniter Forum](http://forum.duniter..org/) for every day talking about development, events, questions & almost every subject. Last, you could join the chatroom with the bottom-right _Chat_ icon visible on each page of this website.
